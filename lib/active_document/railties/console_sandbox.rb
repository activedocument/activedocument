...
#
# @raise [ ActiveDocument::Errors::TransactionsNotSupported ] If the cluster
...
def start_sandbox_transaction(client)
  ...
  ActiveDocument::Threaded.set_session(session, client: client)
...
def start_sandbox
  ActiveDocument.persistence_context.client.tap do |client|
