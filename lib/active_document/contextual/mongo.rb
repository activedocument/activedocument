...
module ActiveDocument
  module Contextual
    ...
    class Mongo
      ...
      # Iterate over the context. If provided a block, yield to a ActiveDocument
      ...
      # ActiveDocument field types. Note that the results may include
      ...
      def pluck(*fields)
        ...
        normalized_select = fields.each_with_object({}) do |f, hash|
      ...
      def tally(field)
        ...
        pipeline.unshift('$match' => view.filter) if view.filter.present?
      ...
      #
      # Depending on the ActiveDocument configuration, the scheduled task can be executed
      # immediately on the caller's thread, or can be scheduled for an
      # asynchronous execution.
      #
      # @api private
      def load_async
        @load_async ||= DocumentsLoader.new(view, klass, criteria)
      end
