...
describe ActiveDocument::Criteria::Findable do
  ...
  describe '#find' do
    ...
    context 'when using string ids' do
      ...
      after(:all) do
        Band.field :_id, overwrite: true, type: :bson_object_id, default: -> { BSON::ObjectId.new }
      end
  ...
  describe '#for_ids' do
    ...
    context 'when only 1 id exists' do
      ...
      it 'does not turn the selector into an $in' do
        expect(criteria.selector_render).to eq({ '_id' => id })
      end
    end
  end
  ...
  describe '#from_database_selector' do
    let(:criteria) { ActiveDocument::Criteria.new(Band) }
    let(:result) { criteria.send(:from_database_selector, [1]) }

    context 'when criteria is empty' do
      it 'adds id' do
        expect(result.selector_render).to eq('_id' => 1)
      end
    end
    ...
    context 'when criteria is restricted by id' do
      ...
      it 'adds id' do
        expect(result.selector_render).to eq('_id' => 2, '$and' => [{ '_id' => 1 }])
      end
    end
  end
end
