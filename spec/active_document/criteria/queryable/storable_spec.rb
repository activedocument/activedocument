...
describe ActiveDocument::Criteria::Queryable::Storable do
  ...
  shared_examples_for 'logical operator expressions' do
    ...
    context '$and operator' do
      context '$and to empty query' do
        ...
        it 'adds to top level' do
          expect(modified.selector_render).to eq({ '$and' => [{ 'foo' => 'bar' }] })
        end
      end

      context '$and to query with other keys' do
        let(:query) do
          ActiveDocument::Query.new.where(zoom: 'zoom')
        end
        ...
        it 'adds to top level' do
          expect(modified.selector_render).to eq({
            'zoom' => 'zoom',
            '$and' => [{ 'foo' => 'bar' }]
          })
        end
      ...
      context '$and to query with $and' do
        let(:query) do
          ActiveDocument::Query.new.where('$and' => [{ zoom: 'zoom' }])
        end
      ...
      context '$and to query with $and which already has the given key' do
        let(:query) do
          ActiveDocument::Query.new.where('$and' => [{ foo: 'zoom' }])
        end
      ...
      context '$and to query with $and onto query whose first one is not $and' do
        let(:query) do
          ActiveDocument::Query.new.where({ 'foo' => 'baz' }).where('$and' => [{ zoom: 'zoom' }])
        end
        ...
        it 'adds to existing $and' do
          expect(modified.selector_render).to eq({ '$and' => [{ 'zoom' => 'zoom' }, { 'foo' => 'bar' }], 'foo' => 'baz' })
        end
      end
    end
    ...
    context '$or operator' do
      context '$or to empty query' do
        ...
        it 'adds to top level' do
          expect(modified.selector_render).to eq({ '$or' => [{ 'foo' => 'bar' }] })
        end
      end
      ...
      context '$or to query with $or' do
        let(:query) do
          ActiveDocument::Query.new.where('$or' => [{ zoom: 'zoom' }])
        end
