...
describe ActiveDocument::Criteria::Queryable::Selectable do
  ...
  shared_examples_for 'a hoisting logical operation' do
    ...
    context 'when argument is a Criteria' do
      ...
      it 'combines' do
        expect(result.selector_smash).to eq(
          'hello' => 'world',
          'foo' => 'bar'
        )
      end
    ...
    context 'when provided a single criterion that is handled via Hash' do
      ...
      context 'when the criterion is a time' do
        ...
        it 'keeps argument type' do
          expect(selection.selector_smash['field']['$gte']).to be_a(Time)
        end
      end
    ...
    context 'when provided a nested criterion' do
      ...
      let(:selection) do
        query.send(tested_method, test: { '$elemMatch' => { field: { '$in' => [1, 2] } } })
      end
      ...
      context 'when the new criteria are for the same field' do
        ...
        context 'when criteria are handled via Hash' do
          ...
          context 'criteria are provided in the same hash' do
            let(:selection) do
              query.send(tested_method, field: { '$gt' => 3, '$lt' => 5 })
            end
    ...
    context 'when argument is a Criteria' do
      ...
      context 'different fields' do
        ...
        it 'combines both fields at top level' do
          expect(result.selector_smash).to eq('hello' => 'world', 'foo' => 'bar')
        end
      end
      ...
      context 'same field' do
        ...
        let(:other) do
          ActiveDocument::Query.new.where(hello: /bar/)
        end

        it 'combines fields with $and' do
          expect(result.selector_smash).to eq('hello' => 'world', '$and' => [{ 'hello' => /bar/ }])
        end
      end
    end
        ...
        it 'generates the expected query' do
          expect(query.selector_smash).to eq({
            '$or' => [
              ...
              # in UTC when persisted as dates by ActiveDocument
  ...
  describe '#not' do
    ...
    context 'when negating a field in the original selector' do
      let(:query) do
        ActiveDocument::Query.new('id' => '_id').where(field: 'foo')
      end
    ...
    context 'when argument is a Criteria' do
      ...
      it 'combines' do
        expect(result.selector_smash).to eq('hello' => 'world', 'foo' => { '$ne' => 'bar' })
      end
    end
    ...
    context 'when argument is a simple Criteria with multiple fields' do
      ...
      let(:other) do
        ActiveDocument::Query.new.where(a: 1, b: 2)
      end
    ...
    context 'when argument is a complex Criteria' do
      ...
      let(:other) do
        ActiveDocument::Query.new.where('$nor' => [{ a: 1, b: 2 }])
      end
  ...
  describe '#none_of' do
    ...
    context 'when there is a single predicate' do
      ...
      it 'adds the predicate' do
        expect(query.selector_smash).to eq('$nor' => [{ 'hello' => 'world' }])
      end
    end
    ...
    context 'when argument is a Criteria' do
      ...
      it 'combines' do
        expect(result.selector_smash).to eq('$nor' => [{ 'foo' => 'bar' }], 'hello' => 'world')
      end
    end
    ...
    context 'when argument is a mix of Criteria and hashes' do
      ...
      let(:other2) { { bar: 42 } }
      let(:other3) { ActiveDocument::Query.new.where(a: 2) }
    ...
    context 'when provided multiple criteria' do
      ...
      context 'when a criterion has an aliased field' do
        ...
        it 'adds the $nor selector and aliases the field' do
          expect(selection.selector_smash).to eq('$nor' => [{ '_id' => 1 }])
        end
