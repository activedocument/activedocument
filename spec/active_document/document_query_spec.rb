...
describe ActiveDocument::Document do
  ...
  context 'when projecting with #only' do
    ...
    it 'populates specified fields only' do
      ...
      expect do
        person.age
      end.to raise_error(ActiveDocument::Errors::AttributeNotLoaded, /Attempted to access attribute 'age' on Person which was not loaded/)
    ...
    it 'allows writing omitted fields' do
      ...
      expect do
        person.age
      end.to raise_error(ActiveDocument::Errors::AttributeNotLoaded)
      person.age = 42
  ...
  context 'when projecting with #without' do
    ...
    context 'when the excluded attribute of the association is retrieved' do
      ...
      it 'prohibits the retrieval' do
        expect do
          person.pet.name
        end.to raise_error(ActiveDocument::Errors::AttributeNotLoaded, /Attempted to access attribute 'name' on Animal which was not loaded/)
      end
    end
  end
end
