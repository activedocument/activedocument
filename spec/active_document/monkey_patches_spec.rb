...
RSpec.describe('Do not add monkey patches') do # rubocop:disable RSpec/DescribeClass
  ...
  def active_document_method?(method)
    method.source_location&.first&.include?('/lib/active_document/')
  end

  def added_instance_methods(klass)
    methods = klass.instance_methods.select { |m| active_document_method?(klass.instance_method(m)) }
    methods -= added_instance_methods(Object) unless klass == Object
    methods.sort
  end

  def added_class_methods(klass)
    methods = klass.methods.select { |m| active_document_method?(klass.method(m)) }
