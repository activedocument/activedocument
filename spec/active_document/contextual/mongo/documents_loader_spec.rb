...
describe ActiveDocument::Contextual::Mongo::DocumentsLoader do
  let(:view) do
  ...
  context 'loading documents' do
    let(:view) do
      klass.collection.find(criteria.selector_render, session: criteria.send(:_session))
    end
  ...
  describe '.executor' do
    ...
    context 'when an unknown executor requested' do
      it 'raises an error' do
        ...
        end.to raise_error(ActiveDocument::Errors::InvalidQueryExecutor)
      end
    end
  end
  ...
  describe '.global_thread_pool_async_query_executor' do
    ...
    context 'when global_executor_concurrency option changes' do
      ...
      it 'creates new executor' do
        ...
        ActiveDocument.global_executor_concurrency = 100
