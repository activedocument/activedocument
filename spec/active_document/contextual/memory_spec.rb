...
describe ActiveDocument::Contextual::Memory do
  ...
  shared_examples 'raises an error when no documents' do
    context 'when there are no matching documents' do
      ...
      it 'returns nil' do
        expect do
          context.send(method)
        end.to raise_error(ActiveDocument::Errors::DocumentNotFound, /Could not find a document of class Address./)
      end
    end
  end
    ...
    describe "##{method}" do
      ...
      context 'when there is a collation on the criteria' do
        ...
        it 'raises an exception' do
          expect do
            context.send(method)
          end.to raise_exception(ActiveDocument::Errors::InMemoryCollationNotSupported)
        end
      end
    end
  end
