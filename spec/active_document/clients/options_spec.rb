...
describe ActiveDocument::Clients::Options, retry: 3 do
  ...
  before do
    ...
    ActiveDocument::Clients.clients.clear
  end
  ...
  describe '#with' do
    ...
    context 'when passing some options' do
      ...
      it 'does not set the options on class level' do
        expect(Minim.persistence_context.client.options['database']).to eq('active_document_test')
      end
      ...
      context 'when the options are not valid mongo client options' do
        ...
        it 'raises an error' do
          expect do
            persistence_context
          end.to raise_exception(ActiveDocument::Errors::InvalidPersistenceOption)
        end

        it 'clears the persistence context' do
          begin; persistence_context; rescue ActiveDocument::Errors::InvalidPersistenceOption; end
          expect(Minim.persistence_context).to eq(ActiveDocument::PersistenceContext.new(Minim))
        end
      end
  ...
  describe '.with' do
    ...
    context 'when passing some options' do
      ...
      it 'does not set the options on instance level' do
        expect(test_model.persistence_context.client.database.name).to eq('active_document_test')
      end
      ...
      context 'when the options are not valid mongo client options' do
        ...
        it 'clears the persistence context' do
          begin; persistence_context; rescue ActiveDocument::Errors::InvalidPersistenceOption; end
          expect(test_model.persistence_context).to eq(ActiveDocument::PersistenceContext.new(test_model, test_model.storage_options))
        end
      end
