...
describe ActiveDocument::Clients::Sessions do
  ...
  before(:all) do
    if Gem::Version.new(Mongo::VERSION) >= Gem::Version.new('2.6')
      ...
      ActiveDocument::Clients.clients.values.each(&:close)
  ...
  after(:all) do
    if Gem::Version.new(Mongo::VERSION) >= Gem::Version.new('2.6')
      ActiveDocument::Clients.with_name(:other).close
      ActiveDocument::Clients.clients.delete(:other)
    end
  end

  let(:subscriber) do
    ActiveDocument::Clients.with_name(:other).send(:monitoring).subscribers['Command'].find do |s|
  ...
  context 'when a transaction is used on a model class' do
    ...
    context 'when transactions are supported' do
      ...
      around do |example|
        ...
        ActiveDocument::Clients.with_name(:other).command(create: :posts)
        ActiveDocument::Clients.with_name(:other).command(create: :canvases)
      ...
      context 'when ActiveDocument::Errors:Rollback raised' do
        let!(:error) do
          capture_exception do
            Person.transaction do
              Person.create!
              raise ActiveDocument::Errors::Rollback
            end
          end
        end
  ...
  context 'when a transaction is used on ActiveDocument module' do
    let(:subscriber) do
      ActiveDocument::Clients.with_name(:default).send(:monitoring).subscribers['Command'].find do |s|
        s.is_a?(EventSubscriber)
      end
    end
    ...
    before do
      ...
      ActiveDocument::Clients.with_name(:default).subscribe(Mongo::Monitoring::COMMAND, EventSubscriber.new)
      subscriber.clear_events!
    end
    ...
    context 'when transactions are supported' do
      ...
      context 'When an error raised' do
        ...
        it 'passes on the error' do
          expect(error).to be_a(ActiveDocument::Errors::Validations)
        end
