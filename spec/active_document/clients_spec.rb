...
describe ActiveDocument::Clients do
  ...
  describe '#collection_name' do
    ...
    context 'when overriding the persistence options' do
      ...
      context 'when outer #with call specifies the collection' do
        use_spec_active_document_config
  ...
  describe '#database_name' do
    ...
    context 'when overriding using the client' do
      ...
      before do
        ActiveDocument.clients[client_name] = { database: database_id_alt, hosts: SpecConfig.instance.addresses }
      end

      after do
        ActiveDocument.clients.delete(client_name)
      end
  ...
  describe '#mongo_client' do
    ...
    before do
      ...
      ActiveDocument.load!(file, :test)
      ActiveDocument.clients[:default][:database] = database_id
    end
    ...
    context 'when getting the default' do
      ...
      it "sets the platform to ActiveDocument's platform constant" do
        expect(mongo_client.options[:platform]).to eq(ActiveDocument::PLATFORM_DETAILS)
      end
  ...
  describe '.store_in' do
    ...
    context 'when provided a non hash' do
      ...
      it 'raises an error' do
        ...
        end.to raise_error(ActiveDocument::Errors::InvalidStorageOptions)
      end
    end
  ...
  describe '.with' do
      ...
      describe '.create!' do
        ...
        context 'when a validation error occurs' do
          ...
          it 'raises the validation error' do
            ...
            end.to raise_error(ActiveDocument::Errors::Validations)
          end
        end
      end
    ...
    context 'when requesting named client' do
      ...
      before do
        expect(ActiveDocument::Clients::Factory).to receive(:create)
          .with(:secondary)
  ...
  context 'when overriding the default database' do
    ...
    before do
      ActiveDocument::Config.load!(file, :test)
    end

    context 'when the override is global' do

      before do
        ActiveDocument.override_database(:active_document_optional)
      end
      ...
      it 'persists to the overridden database' do
        Band.mongo_client.with(database: :active_document_optional) do |sess|
          expect(sess[:bands].find(name: 'Tool')).to_not be_nil
        end
      end

      it 'uses that database for the model mongo_client' do
        expect(Band.mongo_client.database.name).to eq('active_document_optional')
      end
    end
  end
