...
describe ActiveDocument::Document do
  ...
  describe ".included" do
    ...
    let(:new_model) do
      new_klass.tap do
        new_klass.send(:include, ::ActiveDocument::Document)
      end
    end

    let(:twice_a_new_model) do
      new_klass.tap do
        2.times{ new_klass.send(:include, ::ActiveDocument::Document) }
      end
    end
      ...
      context "when no embedded documents are present" do
        context "when using the default discriminator key" do
          ...
          it "raises an error when inappropriate class is provided" do
            ...
            }.to raise_error(ArgumentError, /A class which includes ActiveDocument::Document is expected/)
          end
        end
