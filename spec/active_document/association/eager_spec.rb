...
describe ActiveDocument::Association::EagerLoadable do
  ...
  describe ".preload" do
    ...
    let(:context) do
      ActiveDocument::Contextual::Mongo.new(criteria)
    end
  ...
  describe ".eager_load" do
    ...
    context "when including one has_many relation" do
      ...
      it "runs the has_many preload" do
        expect(ActiveDocument::Association::Referenced::HasMany::Eager).to receive(:new).with([posts_association], docs).once.and_call_original
    ...
    context "when including multiple relations" do
      ...
      it "runs the has_one preload" do
        expect(ActiveDocument::Association::Referenced::HasOne::Eager).to receive(:new).with([cat_association], docs).once.and_call_original
        context.eager_load(docs)
      end

      it "runs the has_and_belongs_to_many preload" do
        expect(ActiveDocument::Association::Referenced::HasAndBelongsToMany::Eager).to receive(:new).with([houses_association], docs).once.and_call_original
        context.eager_load(docs)
      end
    ...
    context "when including two of the same relation type" do
      ...
      it "runs the has_one preload" do
        expect(ActiveDocument::Association::Referenced::HasOne::Eager).to receive(:new).with([ book_association ], docs).once.and_call_original
        expect(ActiveDocument::Association::Referenced::HasOne::Eager).to receive(:new).with([ cat_association ], docs).once.and_call_original
        context.eager_load(docs)
      end
    end
