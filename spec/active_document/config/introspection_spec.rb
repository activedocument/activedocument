...
describe ActiveDocument::Config::Introspection do

  context 'CONFIG_RB_PATH' do
    it 'refers to an actual source file' do
      expect(File.exist?(ActiveDocument::Config::Introspection::CONFIG_RB_PATH)).to be true
    end
  end
  ...
  describe '#options' do
    ...
    ActiveDocument::Config.defaults.each do |name, default_value|
  ...
  describe ActiveDocument::Config::Introspection::Option do
    let(:option) do
      described_class.new(
        'name', 'default', "   # line 1\n    # line 2\n"
      )
