...
describe ActiveDocument::PersistenceContext do
  ...
  describe '.clear' do
    ...
    context 'when the method throws an error' do
      ...
      let!(:persistence_context) do
        described_class.set(object, options).tap do |cxt|
          allow(cxt).to receive(:client).and_raise(ActiveDocument::Errors::NoClientConfig.new('default'))
        end
      end
    ...
    context 'with reusable client' do
      ...
      before do
        expect(ActiveDocument::Clients).to receive(:with_name).with(:some_client).and_return(client)
        expect(client).to_not receive(:close)
      end
      ...
      context 'when the options are not valid client options' do
        ...
        context 'when the options are not valid extra options' do
          ...
          it 'raises an InvalidPersistenceOption error' do
            expect do
              persistence_context
            end.to raise_error(ActiveDocument::Errors::InvalidPersistenceOption)
          end
        end
      end
    end
      ...
      context 'when there are no options passed to the Persistence Context' do
        ...
        context 'when there is a database override' do
          ...
          it 'uses the database override options' do
            expect(persistence_context.database_name).to eq(ActiveDocument::Threaded.database_override)
          end
        end
      end
    end
  ...
  describe '#client' do
    ...
    before do
      ActiveDocument.clients[:alternative] = { database: :active_document_test, hosts: SpecConfig.instance.addresses }
    end

    after do
      ActiveDocument.clients.delete(:alternative)
    end
