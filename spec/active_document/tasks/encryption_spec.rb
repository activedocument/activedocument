...
describe 'ActiveDocument::Tasks::Encryption' do
  require_enterprise
  ...
  describe '.create_data_key' do
    ...
    context 'when all parameters are correct' do
      context 'when all parameters are provided' do
        ...
        it 'creates a data key' do
          result = ActiveDocument::Tasks::Encryption.create_data_key(
      ...
      context 'when kms_provider_name is not provided' do
        context 'and there is only one kms provider' do
          ...
          context 'without key_alt_name' do
            ...
            it 'creates a data key' do
              result = ActiveDocument::Tasks::Encryption.create_data_key(client_name: :encrypted)
              expect(result).to eq(
                {
                  key_id: Base64.strict_encode64(data_key_id.data),
                  key_vault_namespace: key_vault_namespace,
    ...
    context 'when the client name is incorrect' do
      it 'raises an error' do
        expect do
          ActiveDocument::Tasks::Encryption.create_data_key(kms_provider_name: 'local', client_name: :wrong_client)
    ...
    context 'when the client does not have auto_encryption_options' do
      it 'raises an error' do
        expect do
          ActiveDocument::Tasks::Encryption.create_data_key(kms_provider_name: 'local', client_name: :default)
        end.to raise_error(ActiveDocument::Errors::InvalidAutoEncryptionConfiguration)
      end
    end
    ...
    context 'when key_value_namespace is not set' do
      ...
      it 'raises an error' do
        expect do
          ActiveDocument::Tasks::Encryption.create_data_key(kms_provider_name: 'local', client_name: :encrypted)
        end.to raise_error(ActiveDocument::Errors::InvalidAutoEncryptionConfiguration)
      end
    end
    ...
    context 'when kms_providers does not include used provider' do
      it 'raises an error' do
        expect do
          ActiveDocument::Tasks::Encryption.create_data_key(kms_provider_name: 'aws', client_name: :encrypted)
        end.to raise_error(ActiveDocument::Errors::InvalidAutoEncryptionConfiguration)
      end
    end
  end
