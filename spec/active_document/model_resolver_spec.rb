...
ACTIVE_DOCUMENT_MODEL_RESOLVER_KEY__ = :__separate_instance_spec_key
ActiveDocument::ModelResolver.register_resolver ActiveDocument::ModelResolver.new, ACTIVE_DOCUMENT_MODEL_RESOLVER_KEY__
...
describe ActiveDocument::ModelResolver do
  ...
  context 'when using the default instance' do
    ...
    context 'when an alias is not specified' do
      quarantine(self) do
        Object.class_eval <<-RUBY, __FILE__, __LINE__ + 1
          module ActiveDocument; module Specs; module DefaultInstance
            class Vanilla; include ActiveDocument::Document; end
          end; end; end
        RUBY
      end

      let(:model_class) { ActiveDocument::Specs::DefaultInstance::Vanilla }
    ...
    context 'when one alias is specified' do
      ...
      let(:model_class) { ActiveDocument::Specs::DefaultInstance::Aliased }
  ...
  context 'when using a separate instance' do
    let(:resolver) { described_class.resolver(ACTIVE_DOCUMENT_MODEL_RESOLVER_KEY__) }
    ...
    context 'when an alias is not specified' do
      quarantine(self) do
        Object.class_eval <<-RUBY, __FILE__, __LINE__ + 1
          module ActiveDocument; module Specs; module SeparateInstance
            class Vanilla
              include ActiveDocument::Document
              identify_as resolver: ACTIVE_DOCUMENT_MODEL_RESOLVER_KEY__
            end
          end; end; end
        RUBY
      end

      let(:model_class) { ActiveDocument::Specs::SeparateInstance::Vanilla }
      ...
      quarantine(self) do
        Object.class_eval <<-RUBY, __FILE__, __LINE__ + 1
          module ActiveDocument; module Specs; module SeparateInstance
            class Aliased
              include ActiveDocument::Document
              identify_as 'aliased', resolver: ACTIVE_DOCUMENT_MODEL_RESOLVER_KEY__
            end
          end; end; end
        RUBY
      end

      let(:model_class) { ActiveDocument::Specs::SeparateInstance::Aliased }
      ...
      quarantine(self) do
        Object.class_eval <<-RUBY, __FILE__, __LINE__ + 1
          module ActiveDocument; module Specs; module SeparateInstance
            class AliasedMultiple
              include ActiveDocument::Document
              identify_as 'aliased', 'alias2', 'alias3', resolver: ACTIVE_DOCUMENT_MODEL_RESOLVER_KEY__
            end
          end; end; end
        RUBY
      end
