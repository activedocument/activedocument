  ...
  describe '#files_under_path' do
    ...
    context 'when ignore_patterns is empty' do
      ...
      it 'returns all ruby files' do
        ...
        expect(results).to include('rails/active_document')
      end
    end
    ...
    context 'when ignore_patterns is not empty' do
      ...
      it 'omits the ignored paths' do
        expect(results.length).to be > 10 # should be a bunch of them
        expect(results).to_not include('rails/active_document')
      end
    end
  end

  describe '#files_under_paths' do
    let(:paths) { [lib_dir.join('active_document'), lib_dir.join('rails')] }
    ...
    context 'when ignore_patterns is empty' do
      ...
      it 'returns all ruby files' do
        ...
        expect(results).to include('generators/active_document/model/model_generator')
    ...
    context 'when ignore_patterns is not empty' do
      ...
      it 'returns all ruby files' do
        expect(results.length).to be > 10 # should be a bunch
        expect(results).to_not include('generators/active_document/model/model_generator')
