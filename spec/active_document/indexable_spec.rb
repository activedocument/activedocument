...
describe ActiveDocument::Indexable do
  ...
  describe '.included' do
    ...
    let(:klass) do
      Class.new do
        include ActiveDocument::Indexable
      end
    end
    ...
    context 'when database specific options exist' do
      ...
      let(:klass) do
        Class.new do
          ...
          index({ test: 1 }, { database: 'active_document_optional' })
          index({ name: 1 }, { sparse: true })
        end
      end
      let(:indexes) do
        klass.with(database: 'active_document_optional') do |klass|
  ...
  describe '.index' do
    ...
    context 'when providing database option' do
      ...
      before do
        klass.index({ name: 1 }, database: 'active_document_index_alt')
      end
      ...
      it 'sets the index with database option' do
        expect(options).to eq(database: 'active_document_index_alt')
      end
    end
    ...
    context 'when using wildcard indexes' do
      ...
      context 'when not using an alias' do
        ...
        it 'creates the index' do
          expect(spec).to be_a(ActiveDocument::Indexable::Specification)
        end
    ...
    context 'when providing an invalid option' do
      ...
      it 'raises an error' do
        ...
        end.to raise_error(ActiveDocument::Errors::InvalidIndex)
      end
    end
