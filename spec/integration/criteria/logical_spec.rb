...
describe 'Criteria logical operations' do
  ...
  describe '.all_of' do
    ...
    it 'combines existing `$and` clause in query and `where` condition' do
      ...
      expect(bands.selector_render).to eq({
        '_id' => 1,
        'year' => { '$in' => [2020] },
    ...
    context 'when using a symbol operator' do
      context 'when field has a serializer' do
        ...
        it 'sets the $or selector' do
          ...
          expect(scope.criteria.selector_render['$or'].first['created_at']['$lte']).to be_a(Time)
          expect(scope.criteria.selector_render['$or'].last['foo']).to eq('bar')
    ...
    context 'when using a symbol operator' do
      context 'when field has a serializer' do
        ...
        it 'sets the $or selector' do
          ...
          # input was converted from DateTime to Time
          expect(scope.criteria.selector_render['$nor'].first['created_at']['$gt']).to be_a(Time)
          expect(scope.criteria.selector_render['$nor'].last['foo']).to eq('bar')
          expect(scope.to_a).to eq([doc])
        end
      end
    end
  ...
  describe '.not' do
    context 'hash argument with string value' do
      let(:actual) do
        Band.not(name: 'test').selector_render
      end
